...
@dataclass
class FashionCfg(CocoCfg):
    num_classes: int = 4
    splits: Dict[str, dict] = field(default_factory=lambda: dict(
        train=dict(ann_filename='annotations/instances_train.json', img_dir='train', has_labels=True),
        val=dict(ann_filename='annotations/instances_valid.json', img_dir='valid', has_labels=True),
        test=dict(ann_filename='annotations/instances_test.json', img_dir='test', has_labels=False),
    ))
